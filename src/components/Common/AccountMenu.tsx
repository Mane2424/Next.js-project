...
import { menuData } from "@/staticData/sidebarData";
import { usePathname, useRouter } from "next/navigation";
import { logout } from "@/services/auth.service";

const AccountMenu = ({ user, toggleDrawer, setOpenModal, profilePic }: any) => {
	const pathname = usePathname();
	const sidebarData = menuData;
	...
	return (
		<>
			<div className='mb-2 flex items-center gap-1 border-b border-stroke px-6 pb-3.5 dark:border-stroke-dark'>
				<div className=''>
					<img
						src={userProfilePic}
			...
			<div>
				<ul className=''>
					{sidebarData?.map((item: any) => (
						<li
							key={item?.id}
							className='mx-2.5 mb-1 cursor-pointer'
					...
					<li className='mt-2.5 border-t border-stroke pt-2.5 dark:border-stroke-dark'>
						<button className='mx-2.5 w-[92%]' onClick={logOut}>
